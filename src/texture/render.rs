use crate::{Block, Player, Texture, World};
...
pub fn build_chunk(
	...
	let chunk_offset: Vec3 = vec3(x as f32 * 16.0, y as f32 * 16.0, z as f32 * 16.0);

	for (z, y, x) in (0..16).flat_map(|z: usize| {
		(0..16)
			.flat_map(move |y: usize| (0..16)
			.map(move |x: usize| -> (usize, usize, usize) { (z, y, x) }))
	}) {
		let block: Block = data[z][y][x];
		if block == Block::Air {
			continue;
		}

		for i in 0..4 {
			if counter[i] > 9990 || indices[i].len() > 4990 {
				meshes.push(Mesh {
...
fn build_left(
	...
	vertices.push(create_vertex(p[0], vec2(1.0, 1.0)));
	vertices.push(create_vertex(p[1], vec2(0.0, 1.0)));
	vertices.push(create_vertex(p[2], vec2(0.0, 0.0)));
...
fn build_right(
	...
	vertices.push(create_vertex(p[4], vec2(1.0, 1.0)));
	vertices.push(create_vertex(p[5], vec2(0.0, 1.0)));
	vertices.push(create_vertex(p[6], vec2(0.0, 0.0)));
	vertices.push(create_vertex(p[7], vec2(1.0, 0.0)));
...
fn build_front(
	...
	vertices.push(create_vertex(p[0], vec2(0.0, 1.0)));
	vertices.push(create_vertex(p[3], vec2(0.0, 0.0)));
	vertices.push(create_vertex(p[7], vec2(1.0, 0.0)));
	vertices.push(create_vertex(p[4], vec2(1.0, 1.0)));
...
fn build_back(
	...
	vertices.push(create_vertex(p[1], vec2(0.0, 1.0)));
	vertices.push(create_vertex(p[2], vec2(0.0, 0.0)));
	vertices.push(create_vertex(p[6], vec2(1.0, 0.0)));
	vertices.push(create_vertex(p[5], vec2(1.0, 1.0)));
